...
const methodLabel = (m) => (m === 'card' ? 'Card' : m === 'cash' ? 'Cash' : m || 'Other')
...
function reportHtml(data) {
  ...
  const summaryBlock = sec.summary
    ? `<div class="cards">
        <div class="card"><div class="lbl">Revenue</div><div class="amt">${m(data.summary.total)}</div></div>
        <div class="card"><div class="lbl">Orders</div><div class="amt">${data.summary.count}</div></div>
        <div class="card"><div class="lbl">Avg order</div><div class="amt">${m(data.summary.avg)}</div></div>
      </div>`
    : ''
  ...
  const trendBlock = sec.trend
    ? `<h2>Sales trend — by ${data.trendBy === 'month' ? 'month' : 'day'}</h2>
       <table><thead><tr><th>Period</th><th class="r">Orders</th><th class="r">Revenue</th></tr></thead>
       <tbody>${trendRows || '<tr><td colspan="3" class="muted">No sales in this range.</td></tr>'}</tbody></table>`
    : ''
  ...
  const topBlock = sec.topProducts
    ? `<h2>Top products</h2>
       ...
       <tbody>${topRows || '<tr><td colspan="4" class="muted">No sales in this range.</td></tr>'}</tbody></table>`
    : ''
  ...
  const serverBlock = sec.byServer && data.isAdmin
    ? `<h2>Sales by server</h2>
       ...
       <tbody>${serverRows || '<tr><td colspan="3" class="muted">No sales in this range.</td></tr>'}</tbody></table>`
    : ''

  const payRows = (data.payments?.methods || [])
    .map((p) => `<tr><td>${esc(methodLabel(p.method))}</td><td class="r">${m(p.amount)}</td></tr>`)
    .join('')
  const paysBlock = sec.payments
    ? `<h2>Payments</h2>
       <table><thead><tr><th>Method</th><th class="r">Collected</th></tr></thead>
       <tbody>${payRows || '<tr><td colspan="2" class="muted">No payments in this range.</td></tr>'}
       <tr><td>Change given</td><td class="r">${m(data.payments?.change || 0)}</td></tr></tbody></table>`
    : ''
    ...
    * { box-sizing: border-box; }
    ...
    <div class="head">
      <div>
        ...
        <div style="font-size:13px;color:#777;margin-top:2px;">Sales report · ${esc(data.rangeLabel || '')}</div>
      </div>
      <div class="meta">
        <div>For: <b>${esc(data.scope)}</b></div>
