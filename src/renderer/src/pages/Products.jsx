...
export default function Products() {
  ...
  return (
    <div className="h-full flex">
      {/* categories rail */}
      <div className="w-72 shrink-0 bg-surface/60 border-e border-line p-4 flex flex-col gap-2 overflow-y-auto">
        <h2 className="font-display text-lg font-bold mb-1 px-1">{t('products.categories')}</h2>
        <button
          onClick={() => setSelectedCat(null)}
          className={`text-start px-4 py-3 rounded-2xl font-semibold transition ${selectedCat === null ? 'bg-ember text-[#2a1c0c]' : 'bg-surface2 hover:bg-surface3'}`}
        >
          ...
          return (
            <div key={c.id} className="flex items-center gap-1">
              <button
                ...
                className={`flex-1 text-start px-4 py-3 rounded-2xl font-semibold flex items-center gap-2.5 transition ${selectedCat === c.id ? 'bg-ember text-[#2a1c0c]' : 'bg-surface2 hover:bg-surface3'}`}
              >
          ...
          <table className="w-full text-start">
            <thead className="sticky top-0 bg-surface text-muted text-sm z-10">
              <tr>
                ...
                <th className="px-5 py-4 font-medium text-end">{t('products.cols.price')}</th>
            ...
            <tbody>
              {visible.map((p) => (
                <tr key={p.id} className="border-t border-line/60 text-lg hover:bg-surface2/40">
                  ...
                  <td className="px-5 py-3.5 text-end font-display font-bold text-ember tnum">{money(p.price)}</td>
                  ...
                  <td className="px-5 py-3.5 text-end whitespace-nowrap">
                    <button className="px-3 py-2 rounded-xl bg-surface2 border border-line me-2 hover:bg-surface3" onClick={() => setEditing({ ...p, priceStr: String(p.price / 1000) })}>{t('common.edit')}</button>
          ...
          <div className="flex flex-wrap gap-2 mb-2">
            {g.options.map((o) => (
              <span key={o.id} className="chip bg-surface3 text-cream">
                ...
                <button className="ms-1 text-muted hover:text-berry" onClick={() => removeOption(o.id)}>✕</button>
              </span>
            ))}
          </div>
